import React from "react";
import ProjectTask from "./ProjectTask";
import classes from "./Project.module.css";

function Project(props) {
  const tasks = Object.keys(props.data.tasks);
  return (
    <React.Fragment>
      <h2 className={classes["project-name"]}>
        <a href={`${props.data.link}`} target="_blank" rel="noopener noreferrer" className={classes["project-link"]}>
          <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="px" width="16" height="16" viewBox="0 0 16 13">
            <path d="M 9 2 L 9 3 L 12.292969 3 L 6.023438 9.273438 L 6.726563 9.976563 L 13 3.707031 L 13 7 L 14 7 L 14 2 Z M 4 4 C 2.894531 4 2 4.894531 2 6 L 2 12 C 2 13.105469 2.894531 14 4 14 L 10 14 C 11.105469 14 12 13.105469 12 12 L 12 7 L 11 8 L 11 12 C 11 12.550781 10.550781 13 10 13 L 4 13 C 3.449219 13 3 12.550781 3 12 L 3 6 C 3 5.449219 3.449219 5 4 5 L 8 5 L 9 4 Z"></path>
          </svg>
        </a>
        &nbsp;
        {props.data.name}
      </h2>
      {tasks.map((task) => (
        <React.Fragment key={task}>
          <ProjectTask data={props.data.tasks[task]}></ProjectTask>
        </React.Fragment>
      ))}
    </React.Fragment>
  );
}

export default Project;